 ...
 * @Description : fmbMode
 * @Modification Information @ 개발모드 @ 작업일 작성자 내용 @ ---------- ---------
 * ------------------------------- @ 2017.05.29 정유경 최초생성 @ 2017.06.19 jy CHO 수정
 * 
 */
...
angular
		...
		.controller(
				...
				[
						...
						'$rootScope',
						'$location',
						function(CmmAjaxService, CmmWorkerSrvc, CmmFactSrvc,
								$http, $scope, $mdSidenav, $filter, $window,
								$mdDialog, $timeout, $rootScope, $location) {
							/*----------------------------------------------------------------
							 *  변수 선언
							 * ----------------------------------------------------------------
							 *  @ 설명
							...
							$scope.buttons = [ {
								...
								'name' : 'Blue25',
								'value' : 'assets/img/button/Blue25.png'
							}, {
								'name' : 'Red25',
								'value' : 'assets/img/button/Red25.png'
							}, {
								'name' : 'GreenQ',
								'value' : 'assets/img/button/GreenQ.png'
							}, {
								'name' : '직접입력',
								'value' : ''
							} ]
							...
							self.crtEqptModal = function() {
								if (self.eqptParamVo.eqptType == "PLC") {
									...
									var tmp = {}, res = [];
									for (var i = 0; i < self.spcList.length; i++)
										tmp[self.spcList[i].plcId] = 1;
									for (var i = 0; i < self.eqptList.length; i++) {
										if (tmp[self.eqptList[i].id])
											delete tmp[self.eqptList[i].id];
									}
									...
									var tmp = {}, res = [];
									for (var i = 0; i < self.andonList.length; i++)
										tmp[self.andonList[i].plcId] = 1;
									for (var i = 0; i < self.eqptList.length; i++) {
										if (tmp[self.eqptList[i].id])
											delete tmp[self.eqptList[i].id];
									}
							...
							self.saveEqptData = function() {
								...
								var eqptPromise = CmmAjaxService.save(
										"bas/saveFmbEqpt.do", self.eqptList);
							};

							// 설비 이미지리스트 가져오기
							function getEqptList() {
								var eqptPromise = CmmAjaxService.select(
										"bas/selectFmbEqpt.do",
										self.eqptParamVo);
							...
							function getPlcList() {
								...
								var plcPromise = CmmAjaxService.select(
										"bas/selectFmbPlc.do", self.plcParamVo);
								plcPromise
										.then(
												function(data) {
													var dataList = [];
							...
							function getAndonList() {
								...
								var andonPromise = CmmAjaxService.select(
										"bas/selectFmbAndon.do",
										self.andonParamVo);
							...
							function getSpcList() {
								...
								var spcPromise = CmmAjaxService.select(
										"bas/selectFmbPlc.do", self.spcParamVo);
							...
							function getCountList() {
								...
								var countPromise = CmmAjaxService.select(
										"bas/selectFmbLine.do",
										self.countParamVo);
							...
							function getBgImageList() {
								...
								var bgImagePromise = CmmAjaxService.select(
										"bas/selectFmbBgImage.do", self.BgList);
								bgImagePromise
										.then(
												function(data) {
													...
													for (var i = 0; i < self.bgImageList.length; i++) {
														...
														if (factId == "A") {
															...
															$scope.eachBg.D = self.bgImageList[i].imgPath;
														} else if (factId == "Comb") {
							...
							/*
							 * function Fmb007Ctrl ( $scope ) { $scope.data = {
							 * selectedIndex: 0 , bottom: false }; $scope.next =
							 * function() { $scope.data.selectedIndex =
							 * Math.min($scope.data.selectedIndex + 1, 2) ; };
							 * $scope.previous = function() {
							 * $scope.data.selectedIndex =
							 * Math.max($scope.data.selectedIndex - 1, 0); }; }
							 */
							// 배경이미지 변경하기
							// 설비리스트 다시불러오기
							function changeFact() {
								...
								self.spcParamVo.factId = factId;
								self.andonParamVo.factId = factId;

								self.countParamVo.factId = factId;
							...
							// 탭칸에 요소들을 추가시킴.
							self.setSelectedData = function setSelectedData(
									index) {
							...
							self.saveConfig = function(eqpt) {
								var cnm = eqpt['cnm'];
								var target = $filter('filter')(self.eqptList, {
									eqptCnm : cnm
								});
								...
								target[0]['stsImg1'] = $scope.configSetting.imgUrl
										.replace('color', 'Green');
								target[0]['stsImg2'] = $scope.configSetting.imgUrl
										.replace('color', 'White');
								target[0]['stsImg3'] = $scope.configSetting.imgUrl
										.replace('color', 'Blue');
								target[0]['stsImg4'] = $scope.configSetting.imgUrl
										.replace('color', 'Red');

								if (target[0]['status'] != 'delete'
										&& target[0]['status'] != 'insert') {
										...
										.show(
												{
													...
													controllerAs : 'vm',
													templateUrl : 'modules/fmb/views/upload.tmpl.html',
													parent : angular
										...
										.show(
												{
													...
													controllerAs : 'vm',
													templateUrl : 'modules/fmb/views/imageViewer.tmpl.html',
													parent : angular
															.element(document.body),
													clickOutsideToClose : true,
													locals : {
